...
import (
	...
	"github.com/yi-nology/common/utils/xproc"
...
var (
	...
	emptyValue          = reflect.ValueOf(xlang.Placeholder)
)
...
func (u *Unmarshaler) fillMapFromString(value reflect.Value, mapValue interface{}) error {
	...
	case fmt.Stringer:
		if err := xjson.UnmarshalFromString(v.String(), value.Addr().Interface()); err != nil {
			return err
		}
	case string:
		if err := xjson.UnmarshalFromString(v, value.Addr().Interface()); err != nil {
			return err
		}
	default:
		return errUnsupportedType
...
func (u *Unmarshaler) fillSliceFromString(fieldType reflect.Type, value reflect.Value,
	...
	switch v := mapValue.(type) {
	case fmt.Stringer:
		if err := xjson.UnmarshalFromString(v.String(), &slice); err != nil {
			return err
		}
	case string:
		if err := xjson.UnmarshalFromString(v, &slice); err != nil {
			return err
		}
	default:
		return errUnsupportedType
...
func (u *Unmarshaler) fillSliceWithDefault(derefedType reflect.Type, value reflect.Value,
	...
	if !ok {
		...
		} else if err := xjson.UnmarshalFromString(defaultValue, &slice); err != nil {
			return err
		}
...
func (u *Unmarshaler) processNamedFieldWithValueFromString(fieldType reflect.Type, value reflect.Value,
	...
	if len(options) > 0 {
		...
		if !xstrings.Contains(options, checkValue) {
