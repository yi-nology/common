...
func parseKeyAndOptions(tagName string, field reflect.StructField) (string, *fieldOptions, error) {
	...
	cacheLock.RUnlock()
	if ok {
		return xstrings.TakeOne(cache.key, field.Name), cache.options, cache.err
	}
	...
	return xstrings.TakeOne(key, field.Name), options, err
}
