...
// AuthError 表示认证失败错误（401/403），调用方可据此判断是否需要刷新 Token 重试
...
func (e *AuthError) Error() string {
	return fmt.Sprintf("认证失败, 状态码: %d, 响应: %s", e.StatusCode, e.Message)
}

// IsAuthError 判断 error 是否为 AuthError 类型
...
// checkAuthError 检查响应状态码，如果是 401/403 返回 AuthError
func checkAuthError(statusCode int, body string) error {
	if statusCode == 401 || statusCode == 403 {
		return &AuthError{StatusCode: statusCode, Message: body}
	}
	return nil
}
...
func (c *Client) doGet(path string, result interface{}) error {
	if c.Token == "" {
		return &AuthError{StatusCode: 0, Message: "token为空，请先调用GetToken获取token"}
	}
	...
	if !resp.IsSuccessState() {
		if authErr := checkAuthError(resp.StatusCode, resp.String()); authErr != nil {
			return authErr
		}
...
func (c *Client) getRawBytes(path string) ([]byte, error) {
	if c.Token == "" {
		return nil, &AuthError{StatusCode: 0, Message: "token为空，请先调用GetToken获取token"}
	}
